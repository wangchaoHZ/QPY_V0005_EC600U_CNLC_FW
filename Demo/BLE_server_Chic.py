...
def ble_callback(args):
    ...
    elif event_id == event.BLE_CONNECT_IND:  # ble connected
        if status == 0:
...
def ble_gatt_add_characteristic():
    server_id = 0x01
    chara_id = 0x01
    chara_prop = 0x02 | 0x08 | 0x10 | 0x20  # 0x02-可读 0x08-可写 0x10-通知 0x20-指示
    uuid_type = 1  # 短UUID
    uuid_s = 0x8002
    uuid_l = bytearray([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
...
def ble_gatt_add_characteristic_value():
    ...
    uuid_s = 0x8002
    uuid_l = bytearray([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
    data = [0x37 for i in range(244)] # data长度最长244字节
...
def ble_gatt_send_notification():
    ...
    # data = [0x39 for i in range(244)] # data长度最长182字节
    data = [0x39 for i in range(182)]  # data长度最长182字节
    # data = [0x39 for i in range(20)]
    value = bytearray(data)
